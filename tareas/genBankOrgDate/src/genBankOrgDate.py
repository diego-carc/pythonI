...
DESCRIPTION
    ...
    para obtener el organismo al que pertenece así como la fecha de creación.
...
USAGE
    ...
    % python genBankOrgDate.py [-h] [-g GENBANK] [-f] [-e EMAIL]
...
if args.file:
    ...
    with Entrez.efetch("nucleotide", id=args.genBank, rettype="gb") as handle:
         for gb_record in SeqIO.parse(handle, format="genbank"):
              for k,v in gb_record.annotations.items():
                   if k in ["date", "organism"]:
                       print(f"{k} : {v}")
    exit()
